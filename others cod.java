...
// TODO code application logic here
	long startTime = System.currentTimeMillis();
		...
		try{
			...
			/*
				...
				while ((input_str = l_lnr.readLine()) != null)
				{
					...
					{
						if (input_str.charAt(0) == ＆C＊)
						{
							circuit_count++;
						}
						else if (input_str.charAt(0) == ＆J＊)
						{
							juggle_count++;
						}
					}
			...
			*/
				...
				while ((input_str = lnr.readLine()) != null)
				{
					if (!input_str.isEmpty())
					{
						...
						if ( size < 5 )
						{
							throw new IllegalArgumentException(※Invalid Input File incorrect number of input per line§);
						}
						else
						{
							//C C0 H:10 E:8 P:10
							...
							if (flag.equals(※C§))
							{
								...
								{
									throw new IllegalArgumentException(※Invalid Input File Circuit index wrong§);
								}
								circuit_count++;
							} else if (flag.equals(※J§))
							{
								if (juggle_count == n)
								{
									...
									//int size = values.length;
									j.set_Pref_Listsize(size 每 5);
									for (int i = 0; i < size 每 5; i++)
									{
										int tmp = Integer.parseInt(values[5 + i].substring(1));
										if (tmp == circuit_list.get(tmp).getIndex())
										{
											j.addpref_Circuit(tmp);
										} else
										{
											throw new IllegalArgumentException(※Invalid Input File Juggle index wrong§);
										}
									}
									juggle_list.add(j);
								} else
							...
							{
								throw new IllegalArgumentException(※Invalid Input File not C or J§);
							}
						}
					}
				}//end of while loop
			...
			//based on the previous input file, init the circuit and juggler＊s list info
			//and calculate the dot product value
				for (int i= 0; i <circuit_count; i++ )
				{
					...
					for (int j= 0; j <juggle_count; j++ )
					{
						...
						tmp_circuit.add_Jvalue(tmp_juggler.get_Juggler_Index(), result);
					}
				}
			...
			//Now we have finished retrieving informatin from the iputfile, time to solve the problem
				Find_Solution f_solution = new Find_Solution (circuit_list, juggle_list);
		...
		long totalTime = endTime 每 startTime;
		System.out.println(※Run for § + totalTime);
	}
}

/* this class hold the current solution information
* including a array of currently find solution list (circuit, juggler)
* a array of for jugglers showing whether a juggler is taken or not,
* if it is not taken, the value is -1, if it is taken, then the index
* number of the circuit
* an array of counter for each circuit＊s remaining spots
*/
class Current_Solution_Info
{
	//this two dimensianl array holds the information for assigned jugglers for each circuit
	private int[][] circuit_j_list;

	//this is a array holds the information for number of remainning spot for each circuit
	private int[] circuit_left_spots;

	//this is an array that hold each juggler＊s information. If it is not taken, -1, if it is taken the circuit index
	//it is assigned to
	...
	//why 1970? someone was born 1970? I like 1990 way better, caz that is the year my gf was born
	...
	public Current_Solution_Info (ArrayList<Circuit> c_list, ArrayList<Juggler> j_list)
	{
		circuit_list = c_list;
		juggle_list = j_list;
	...
	//return true if the input j is a bitter fit for some juggler in this c circuit
	...
	//always start check from bottom
		for ( int i = juggle_count/circuit_count -1 ; i >= 0; i每)
		{
			int check_j_num = circuit_j_list[c][i];
			int check_j_value = circuit_list.get(c).return_Product_Value(check_j_num);
	...
	// 		System.out.println (※Add ※+ j + § juggler into § + c + § circuit§);
		int index = juggle_count/circuit_count 每 circuit_left_spots[c];
	...
	//remain sorted
		...
		for (int i = index 每 1 ; i >= 0; i每)
		{
			int cur_j_index = circuit_j_list[c][i];
			int cur_j_value = circuit_list.get(c).return_Product_Value(cur_j_index);
	...
	{
		int index = juggle_count / circuit_count 每 1;
		int j = circuit_j_list[c][index];
	//System.out.println (※Remove ※+ j + § juggler from § + c + § circuit§);
	...
	public void debugSolution (int c_index)
	{
		...
		int num = juggle_count/circuit_count 每 circuit_left_spots[c_index];
		for (int i = 0 ; i <num; i++)
	...
	public boolean verifySolution ()
	{
		for (int k = 0; k< juggle_count; k++)
		{
			...
			for (int i=0;i< size;i++)
			{
				...
				int my_pref_value = circuit_list.get(cur_pref).return_Product_Value (k);
	//now check all the selected jugglers for this cricuit cur_pref
				...
				for (int j = 0; j < j_count;j++)
				{
					...
					int check_value = circuit_list.get(cur_pref).return_Product_Value (check_j_num);
	...
	//find out the circuit C1970∩s total sum
				for ( int i = 0; i < j_per_c; i++)
				{
					...
					System.out.println (§ § + circuit_j_list[yodle_email_magic_num][i]);
				}
			}
			else
			{
...
class Find_Solution
{
	...
	*/
	...
	{
		Current_Solution_Info cur_sul = new Current_Solution_Info (circuit_list,juggle_list);
	...
	static private int partition(int arr[], int left, int right, int index_array[])
	{
		int i = left, j = right;
		int tmp;
	...
	//i chose to not to use java collection sort for convinience, speed and space
	static public void quickSort(int arr[], int left, int right, int index_array[])
	{
		int index = partition(arr, left, right, index_array);
		if (left < index 每 1)
		{
			quickSort(arr, left, index 每 1, index_array);
		}
		if (index < right)
		{
	...
	private void attemtp_Find_Solution (ArrayList<Juggler> input_juggle_list, Current_Solution_Info cur_sul)
	{
		int tmp_sort_list[];
		...
		//then do a sort
		//quickSort(tmp_sort_value, 0, avail_j_num 每 1, tmp_sort_list);
		
		//starting from top of sort list
		for (int j = 0; j < avail_j_num; j++)
		{
			//System.out.println(※attempting § + j +§ juggler for § + i + § ciruit§);
			//System.out.println(※Current juggler is § + j );
			int attempt_j_num = tmp_sort_list[j];//circuit_list.get(i).return_Sort_List_Index (j);
			//try the circuits for this juggler in the following order
			//first select from the preference list,the first, then second, then third then based on the product dot value for each circuit
	
			int attempt_num = juggle_list.get(attempt_j_num).get_Prev_Circuit_Attempt_Num();
			int attempt_c_num = juggle_list.get(attempt_j_num).get_CurrentTopPickCircuit(attempt_num);
			attempt_num++;
			...
			while (attempt_c_num != -1)
			{
				...
				{
					//just add the juggler straight into the circuit if it is not full
					cur_sul.add_New_Sol(attempt_c_num, attempt_j_num);
					//always set the number of attempts on circuit, so if this juggler got kicked
					//next time he know where to start
					juggle_list.get(attempt_j_num).set_Prev_Circuit_Attempt_Num(attempt_num);
					nsert_j_flag = true;
					break;
				} else
				{
					//this circuit is full
					//remember all the juggers are already sorted in the circuit, so we start from the last one
					//then check all the jugger already asigned to this circuit, if there is any juggler perform worse than me, kick worst one and pick me!
					if (cur_sul.check_juggler_Better_Fit(attempt_c_num, attempt_j_num))
					{
						//we need to remove the last one and add the new juggler into the solution
						int removed_j = cur_sul.remove_Circuit_Last_Jugger(attempt_c_num);
			
						remove_juggle_list.add(juggle_list.get(removed_j));
						//add the new juggler, what a sad day for the one who got kicked
						cur_sul.add_New_Sol(attempt_c_num, attempt_j_num);
				...
				attempt_c_num = juggle_list.get(attempt_j_num).get_CurrentTopPickCircuit( attempt_num);
				attempt_num++;
			}//end of while loop
	
			if (!insert_j_flag)
			{
				//if none of this juggler＊s preference was selected. just pick any circuit base on the product dot
				attempt_num每;
				attempt_c_num = juggle_list.get(attempt_j_num).get_Rest_Non_Pref_PickCircuit(attempt_num);
				attempt_num++;
				while (attempt_c_num != -1)
				{
					if (cur_sul.get_Circuit_Remain_Spots(attempt_c_num) > 0)
...
/*
	public int return_Sort_List_Index (int i)
	{
		return juggle_sort_list[i];
	}
*
*/
	public void circuit_init_list (int jum_per_c, int t)
	{
		...
		//juggle_vlist = new ArrayList<Circuit_Juggle_Value>(total_jugglers_num);
...
class Juggler
{
	...
	public void set_Circuit_List (ArrayList<Circuit> c_list)
	{
		circuit_list = c_list;
	/*
		int size = circuit_list.size();
		//circuit_index_list = new int[size];
		int ciruit_value_list [] = new int [size];
		
		//we sort the circuit in following way, first prefence, second, and so on, and then the product dot value
		for ( int i = 0; i < size; i++)
		{
			//circuit_index_list[i] = i;
			ciruit_value_list[i] = max_dot_product* get_Circuit_PrefBonus(i) +circuit_list.get(i).return_Product_Value(juggler_index);
		}
		
		//Find_Solution.quickSort(ciruit_value_list, 0, size 每 1, circuit_index_list);
	*/
	}
	...
	//this function is used when all the circuits in the juggler＊s preference list have been selected
	public int get_Rest_Non_Pref_PickCircuit (int num_attempt)
	{
		int count = 0;
		int v = num_attempt 每 pref_list_size;
	...
	//get the juggler＊s prefered circuit from his preference list based on the number of attempt
...
/*
	...
	public int get_FirstPref (Current_Solution_Info cur_sul)
	{
		...
		else
		{
			for (int i = 0; i < pref_list_size; i++)
			{
				int c = pref_list.get(i);
				if (cur_sul.get_Circuit_Remain_Spots(c) > 0)
				return c;
			}
			return -1;
		}
